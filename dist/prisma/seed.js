...
const seedDatabase = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const categories = yield seedCourseCategory();
    const users = yield seedUser();
    yield seedCourseAndCategories();
    yield seedChannelAndChats();
    function seedCourseCategory() {
    ...
    function seedCourseAndCategories() {
        return __awaiter(this, void 0, void 0, function* () {
            const courses = [];
            const modules = [];
    ...
    function seedChannelAndChats() {
        return __awaiter(this, void 0, void 0, function* () {
            const channels = [];
            const chats = [];
            for (let i = 0; i < NUM_OF_CHANNELS; i++) {
                const channel = yield index_1.default.channel.create({
                    data: {
                ...
                for (let j = 0; j < NUM_OF_CHATS_EACH; j++) {
                    const chat = yield index_1.default.chatMessage.create({
                        data: {
                            message: faker_1.faker.lorem.paragraph(),
                            channel: { connect: { id: channel.id } },
                            sender: { connect: { id: users[faker_1.faker.number.int({ max: i })].id } },
                        },
                    });
                    chats.push(chat);
                }
